...
const MUTATIONS = {
    ...
    // `Math.max(viewW, viewH)` is the recorded design. Splitting it into a per-axis pair
    // reintroduces sensitivity to the (top, right, bottom, left) argument order (moot when all
    // four are equal) and to which axis's size the pad happens to be derived from. Two anchors,
    // one per declaration site (the mount-time one and applySize's large-mode re-derivation), so
    // both must survive.
    ...
    "size-pad-wrongly-takes-the-x-factor": ["T",
        "viewW = Math.round((cfg.boxW * xf + cfg.padX + (large ? cfg.padXRLarge : cfg.padXR)) * f);",
        "viewW = Math.round((cfg.boxW + cfg.padX + (large ? cfg.padXRLarge : cfg.padXR)) * xf * f);"],
...
const CLIP_B = jsConst(B_SRC, "CLIP_B_REM", "MoEProgress.js");
